...
**Data flow:** `worker/src/feeds.ts` lists 35 hardcoded `{source, url, region, lang}` RSS/Atom feeds (23 `region:"kr"` + 12 `region:"global"`) → an hourly cron (`worker/src/index.ts`, `scheduled()`) fetches and upserts them into the D1 `articles` table (`worker/schema.sql`) → `GET /api/articles` returns the latest 10 per source (all regions) → `extension/src/App.tsx` fetches that on load and renders one column per source.

`App.tsx` has four mutually-exclusive views toggled from the header: the per-source board (default, **KR sources only**), the bookmarks grid (`showBookmarksOnly`), a "latest posts" grid (`showRecentFeed`, opened by the `NEW` button — pulses via `.new-toggle-bounce` when there's a KR article newer than `localStorage["techtab-recent-feed-seen"]`, last 7 days of KR sources, newest first, capped at 40), and a global grid (`showGlobalFeed`, opened by the globe button — the 12 `region:"global"` sources, newest first, capped at 40, no time window). The board / recent / `hasUnseenRecent` all run off `krArticles` (articles minus `GLOBAL_SOURCES`); the global grid off the complement. All three grids reuse the `.board` 5-col grid via the `FlatFeed` component, rendering `ArticleCard`s directly instead of `.column`s.

`newestSeen()` — the "seen" timestamp written for the NEW button / per-source NEW badges is `max(now, newest published_at in the list)`, **not** wall-clock `now`; a feed article dated in the future (publisher post-dating, client clock skew) would otherwise keep NEW stuck on forever. `App.tsx` also refetches `/api/articles` on `visibilitychange`/`focus` (min 5 min apart, list only — no NEW re-detection) so a long-lived open tab doesn't go stale.
...
**i18n is ko/en, split by where the string lives.** UI strings go through `extension/src/i18n.ts` (`useI18n()` + `t()`, a plain dict — no library); locale is `localStorage["techtab-locale"]` or a `navigator.language` ko-prefix check, toggled by the button right of the theme toggle. Brand names and keyword badges translate for **display only** via `sourceLabel()` / `keywordLabel()` — the raw `source` string is never rewritten (it keys localStorage, favicons, and `SOURCE_DOMAINS`). Dates use `Intl.RelativeTimeFormat`. Article titles are translated **server-side, bidirectionally**: `worker/src/index.ts` calls the DeepL Free API (`api-free.deepl.com`, `DEEPL_API_KEY` secret) once per feed per hourly cron, direction keyed off `feed.lang` — `lang:"ko"` feeds translate KO→EN into `title_en`, `lang:"en"` feeds translate EN→KO into `title_ko`. It translates the newest ≤20 rows per source where the target column `IS NULL` (covers new rows + retries past failures + backfills), so a new prod DB needs `ALTER TABLE articles ADD COLUMN title_ko TEXT` first. `App.tsx` `displayTitle()` shows `title_en` in en locale / `title_ko` in ko locale, falling back to `title` when null (so KR articles in ko and global articles in en just show the original). The `docs/` landing page has its own tiny inline-script i18n (`data-i18n` attributes) keyed off the same `techtab-locale`. Store-listing name/description come from `extension/public/_locales/{ko,en}/messages.json` via `__MSG_*__` in the manifest (`default_locale` is `ko`); the CWS dashboard listing copy and per-locale screenshots are separate and managed there.
...
- D1 queries stay parameterized (`.bind()`); never string-concatenate feed data into SQL. (`collectFeeds` interpolates `title_en`/`title_ko` as a column name — that's a fixed literal from `feed.lang`, never external input; keep it that way.)
